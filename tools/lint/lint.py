...
def check_unique_basename_paths(repo_root, paths):
    # type: (str, List[str]) -> List[rules.Error]
    """
    Checks that all files have unique basename paths.

    The 'basename path' refers to the entire path excluding the extension. For
    example, 'foo/bar/baz.html' and 'foo/bar/baz.svg' have the same basename
    path, but 'foo/bar/baz.html' and 'foo/qux/baz.html' do not.

    Files with the same basenames have caused problems in downstream
    infrastructure (see https://github.com/web-platform-tests/wpt/issues/7570),
    and cause confusion in general.
...
all_paths_lints = [check_css_globally_unique, check_unique_basename_paths]
